...
y_vec=np.zeros(40)
abnor_vec=np.zeros(40)
safe_vec=np.zeros(40)
...
while True:
		...
		for i in range(0, D.shape[0]):
			for j in range(i + 1, D.shape[1]):
				...
				if (D[i, j] < config.MAX_DISTANCE) and not serious:
					abnormal.add(i)
					abnormal.add(j)
				# update our safe set if the centroid distance is greater than max distance limit
				if (D[i, j] > config.MAX_DISTANCE) and not serious and not abnormal:
					safe.add(i)
					safe.add(j)
	...
	y_vec[-1] = len(serious)
	abnor_vec[-1]=len(abnormal)
	safe_vec[-1]=len(safe)
	temp,temp2,temp3 = py_live.live_plotter(x_vec,y_vec,abnor_vec,safe_vec,temp,temp2,temp3)
	y_vec = np.append(y_vec[1:],0.0)
	safe_vec = np.append(safe_vec[1:],0.0)
	abnor_vec = np.append(safe_vec[1:],0.0)
